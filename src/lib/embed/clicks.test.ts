...
describe("recordClick", () => {
  it("counts a click and returns where to send the visitor", async () => {
    ...
    expect(await recordClick(listing.id)).toBe("https://github.com/mira-builds");
  ...
  it("counts each click rather than only the first", async () => {
    const listing = await seedListing();
    for (let i = 0; i < 5; i += 1) await recordClick(listing.id);
  ...
  it("returns nothing for a listing that does not exist", async () => {
    expect(await recordClick("00000000-0000-4000-8000-000000000000")).toBeNull();
  });
  ...
  it("reads the destination from the row, never from a caller", async () => {
    ...
    expect(await recordClick(listing.id)).toBe("https://example.com/real-target");
  });
});
