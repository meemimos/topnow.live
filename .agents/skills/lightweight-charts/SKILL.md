...
candles.setData([{ time: 1700000000, open: 1, high: 2, low: 0.5, close: 1.5 }]);
```
...
chart.applyOptions({
  timeScale: {
    timeVisible: true,
    secondsVisible: true,
    tickMarkFormatter: (time) => (typeof time === "number" ? formatTime(time) : String(time)),
  },
  localization: {
    timeFormatter: (time) => (typeof time === "number" ? formatTime(time) : String(time)),
  },
});
```
...
> _"My chart snaps to the right edge every time new data arrives."_
